...
**Playable v1 gameplay loop.** Research, design wireframes, and the master plan are complete. Findings live under `docs/research/`; execution guidance lives under `docs/plan/`. The implementation in this isolated clone now covers the real room/table/phase/bot loop for 4P / 6P / 8P, with remaining work treated as explicit v1.1+/ops deferrals rather than P0 blockers.

Maintain the playable loop with the 2026-05-18 isolation amendment applied:
...
3. ROOM-1 / ROOM-3 / NET-1 / NET-3 scaffolding exists under `api/` and `lib/realtime/`: room lifecycle, public room listing, selectable 4P/6P/8P room capacity, 6P/8P `2-teams-of-n` vs `teams-of-2` room rule support with pair-team progression for teams-of-2 rooms, host-controlled room start with bot fill and kick, started-room locking, lowest-open-seat join allocation, per-player room tokens, move POST, bounded SSE stream polling + replay framing, long-poll replay fallback, idempotency, event logs, hidden-state payload filtering, self-hydrating filtered views on gameplay mutation responses, and Upstash REST-backed persistence adapters.
4. TRIBUTE-1 / EXCHANGE-1 server progression exists for post-round advance, default server-side tribute auto-pick, bot tribute/return/exchange vote/exchange-select automation, tribute selection, return selection, exchange vote, exchange selection, state-node payload filtering, and filtered SSE publish. 6P/8P Path B sweep tribute is constrained to `2-teams-of-n`; teams-of-2 rooms use normal last-to-first tribute. Browser SSE consumes named server event types, and table hydration chooses the newest state-versioned view across SSE and POST responses. S20-S23 tribute/exchange modal surfaces, typed phase-action clients, filtered-view table/overlay adapters, and production two-browser UI smoke coverage exist.
5. AI-1 / AI-3 / AI-4 foundation exists for legal-move enumeration, Easy/Medium bot decisions, hand sorting / move suggestions, token-protected server-side assist suggestions, stable bot identities, human-like timing helpers, dynamic-budget inline bot chaining in `api/move` for 8P bot stretches, automatic bot phase actions after human phase submissions, immediate bot continuation after round/tribute/exchange transitions back to `playing`, `api/tick` continuation, `api/cron/dcCheck` disconnect takeover guarded by `INTERNAL_TICK_SECRET` when configured, and deterministic local self-play round/benchmark harnesses. WASM solver and Elo-rated benchmark league are still pending.
6. SEC foundation includes shared memory / Upstash fixed-window rate limiting wired into core mutation routes, online profile creation with 50-per-IP daily throttle after handle validation, same-room same-IP warning detection, per-player room-token authorization on default SSE/move/round-next/leave/tribute/exchange/assist routes, report/admin moderation endpoints, ban enforcement on room join, an admin dashboard client surface, official `botid` server verification plus header fallback on move/report submissions, browser-side BotID init for protected POST routes, frameworkless Vite BotID rewrites/headers in `vercel.json`, and a friendly automation UA for live smoke.
...
10. Full-game API integration coverage exists for all-human and human+bot 4P/8P games through create/join/start/move/round/tribute APIs to `game-end`; production API smoke covers create/start/poll/SSE/move, and live browser production smoke covers create/join/start/play/sync.
...
2026-05-18 — Playable v1 gameplay loop in the isolated `guandan-online-codex` clone; selectable 4P/6P/8P rooms, 6P/8P two-team vs teams-of-2 room settings, required local player-handle setup, started-room locking, stable seat-id allocation, local active-room reconnect persistence, server-side tribute/exchange progression with default tribute auto-pick and bot phase automation, immediate bot continuation after phase transitions, self-hydrating filtered mutation responses, Easy/Medium bot foundations, server-side assist suggestions, deterministic local self-play and benchmark CLI, full-game 4P/8P API integration tests, Vercel Node API adapter coverage, disconnect bot takeover, backend anti-abuse controls, per-player room-token auth, BotID rewrites, telemetry backend/admin panel, room/admin UI wiring, S20-S23 phase overlays, round-end advance panel, filtered SSE hook, live two-browser production smoke, and active-room table actions are wired.
